...
const IdentificationPage: NextPage = () => {
    ...
    const handleDone = async (accept: boolean) => {
        try {
            setIsLoading2(true);
            setIsDisable(true);
            const data: PharyngitisDiagnosisAcceptance = {diagnosisId: analysisResult?.diagnosisId!, accept: accept};
            const response = await PharyngitisAnalyzeService.pharyngitisDiagnosisAccept(data);
            if (response.success) {
                notifySuccess(response.message);
            }
        } catch (error) {
                        ...
                        {analysisResult ? (
                            analysisResult.prediction === 'invalid'
                                ? <div className="w-full text-gray-700 space-y-4">
                                    ...
                                    <p className="text-gray-500 text-sm">
                                        <strong>Please upload valid Throat Image.</strong>
                                    </p>
