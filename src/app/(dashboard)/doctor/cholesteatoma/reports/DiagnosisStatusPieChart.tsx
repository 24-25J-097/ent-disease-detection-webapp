...
const DiagnosisStatusPieChart: React.FC<{
    diagnosisStatusData: DiagnosisStatusChart[],
    chartRef: any
}> = ({diagnosisStatusData, chartRef}) => {

    const total = diagnosisStatusData.reduce((sum, item) => sum + item.value, 0);

    return (
        <div ref={chartRef} className="bg-white p-6 shadow-lg rounded-xl border border-gray-200">
            <h2 className="text-xl font-semibold text-center mb-4 text-gray-700">Diagnosis Status Overview</h2>
