...
import DiagnosisStatusPieChart from '@/app/(dashboard)/doctor/sinusitis/reports/DiagnosisStatusPieChart';
import SinusitisStagesBarChart from '@/app/(dashboard)/doctor/sinusitis/reports/SinusitisStagesBarChart';
import ConfidenceScoreBarChart from '@/app/(dashboard)/doctor/sinusitis/reports/ConfidenceScoreBarChart';
import DiseaseVsHealthyDoughnutChart
    from '@/app/(dashboard)/doctor/sinusitis/reports/DiseaseVsHealthyDoughnutChart';
...
const ReportsPage: NextPage = () => {
    ...
    useEffect(() => {
        (async () => {
            await fetchSinusitisReportsData();
        })();
    }, []);

    const fetchSinusitisReportsData = async () => {
        try {
            const response = await SinusitisAnalyzeService.getSinusitisReports();
            if (response.success && response.data) {
                setReportsData(response.data);
            }
        } catch (error) {
            console.error("Error in get sinusitis reports data: ", error);
        }
    };
                    ...
                    {dropdownOpen && (
                        <div className="absolute mt-2 w-64 bg-white shadow-lg rounded-md py-2">
                            ...
                            <button
                                ...
                                className="block w-full px-4 py-2 text-left hover:bg-gray-200"
                            >
                                Print Sinusitis vs. Healthy
                            </button>
                            <button
                                onClick={() => handlePrint(stagesBarChartRef)}
                                className="block w-full px-4 py-2 text-left hover:bg-gray-200"
                            >
                                Print Sinusitis Stages
                            </button>
                            <button
